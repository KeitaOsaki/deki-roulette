...
- `src/components/LocaleSwitch.tsx` — 言語切替。
...
- `public/robots.txt` / `public/sitemap.xml` はビルド時に `dist/` へコピーされる。
