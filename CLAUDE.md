...
- 日本語版はブランド名「デキレーレット」を `<title>` / `<h1>` に出す。英語版の `<title>` / `<h1>` は
  一般語「Roulette」のまま。画面の本文には「当たり」「必ず」等、隠し操作の内容そのものを示唆する語は置かない。
...
- 英語版の `<title>` / `<h1>` は一般語のみ（「Roulette」「Free」）。ブランド語は `meta[name=description]` と JSON-LD の `alternateName` にだけ置く。
- どちらの言語でも、隠し操作の手順（長押し）は画面にも meta にも書かない。
